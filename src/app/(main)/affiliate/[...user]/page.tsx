...
export default function SignIn() {
  ...
  const checkValidity = useCallback(async () => {
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/affiliate/check-validity`, {
      method: "POST",
      headers: {
  ...
  const getData = useCallback(async () => {
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/affiliate/get-data`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  useEffect(() => {
    checkValidity().then((valid) => {
      if (valid) getData();
    });
  }, [checkValidity, getData]);
    ...
    <>
      ...
      <div className="w-full h-full flex flex-col items-center justify-start">
        {isValid ? (
          ...
          <main className="w-full h-full text-foreground text-xl">Forbidden</main>
        ) : (
          <main className="w-full h-full text-foreground text-xl"></main>
        )}
      </div>
    </>
  );
