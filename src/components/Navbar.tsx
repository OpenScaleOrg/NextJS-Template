...
const Navbar = async () => {
    ...
    return (
        <nav className='z-50 flex w-full fixed top-0 justify-center md:h-[80px] h-[80px] bg-main-background' id='navbar_container'>
            ...
            <div className='w-full h-full max-w-[1440px] lg:px-20 px-5 lg:py-2.5 py-2.5 flex justify-between'>
                ...
                <ul className='items-center md:gap-[40px] gap-2 font-medium lg:flex hidden'>
                    <li key={crypto.randomUUID()} className='transition ease-in-out hover:animate-text hover:text-[#F00000]'>
                        <Link rel="canonical" className='md:text-[16px] text-main-foreground' href={'/#features'}>Features</Link>
                    </li>
                    <li key={crypto.randomUUID()} className='transition ease-in-out hover:animate-text hover:text-[#F00000]'>
                        <Link rel="canonical" className='md:text-[16px] text-main-foreground' href={'/#pricing'}>Pricing</Link>
                    </li>
                    <li key={crypto.randomUUID()} className='transition ease-in-out hover:animate-text hover:text-[#F00000]'>
                        <Link rel="canonical" className='md:text-[16px] text-main-foreground' href={'/studio'}>Studio</Link>
                    </li>
                </ul>
                ...
                <div className='flex justify-center items-center gap-[20px]'>
                    <ul className='items-center md:gap-[20px] gap-[10px] flex'>
                        ...
                        <div className='items-center md:gap-5 gap-5 flex'>
                            {user ? (
                                <Link href={'/studio'} className={`w-full h-[60px] md:p-4 md:gap-4 gap-2 text-[14px] rounded-[12px] flex items-center cursor-pointer`}>
                                    ...
                                    <div className='w-full flex flex-col justify-center items-start'>
                                        <span className='font-bold text-[14px] text-main-foreground whitespace-nowrap text-ellipsis sm:w-auto w-[60px] overflow-hidden'>{user.firstName} {user.lastName?.charAt(0).toUpperCase()}.</span>
                                        <span className='font-normal text-[12px] text-main-light whitespace-nowrap text-ellipsis sm:w-auto w-[60px] overflow-hidden'>{user.email}</span>
                                    </div>
                                </Link>
                            ) : (
                                <>
