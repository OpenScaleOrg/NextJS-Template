...
const NavbarStudio = async ({ upgrade = true, user }: { upgrade?: boolean; user: User }) => {
	return (
		<nav
			className="z-50 flex w-full fixed top-0 justify-between md:h-[80px] h-[60px] bg-grey bg-main-background border-b-[1px] border-main-light/20 dark:border-main-light/10 md:pr-20 pr-5"
			id="navbar_container"
		>
				...
				<Link rel="canonical" href={'/'} className="flex items-center gap-2 md:gap-4 outline-none">
					<div className="flex items-center md:gap-[10px] h-full">
						<Image
							...
							draggable={false}
							alt={"Frist'OX Studio"}
							className="object-contain pointer-events-none dark:hidden w-[127px] h-[30px]"
						/>
						<Image
							src={'/images/logo-dark.svg'}
							width={127}
							...
							draggable={false}
							alt={"Frist'OX Studio"}
							className="object-contain pointer-events-none dark:block hidden w-[127px] h-[30px]"
						/>
					</div>
				</Link>
			</div>
			...
			<div className="w-full flex-col justify-center items-start md:px-5 px-5 md:flex hidden">
				<span className="w-full text-main-foreground font-extrabold md:text-[20px] text-[12px]">
					Welcome back, {user.firstName} {user.lastName?.charAt(0).toUpperCase()}.
				</span>
				<span className="w-full text-main-foreground md:text-[12px] text-[12px] whitespace-nowrap">
					Ready to bring your 2D design to 3D design life?
				</span>
			</div>
				...
				{upgrade && (
					<>
						{user.plan == 'free' ? (
							<Link
								href={'/studio/upgrade'}
								className="border-[#BFDBFE]/50 border text-main-foreground bg-gradient-to-r from-[#FFFFFF] to-[#FAFAFA] dark:bg-main-foreground/20 dark:from-transparent dark:to-transparent dark:backdrop-blur-sm rounded-full md:py-3 py-2 md:px-6 px-3 flex justify-center items-center md:gap-2 gap-2"
							>
