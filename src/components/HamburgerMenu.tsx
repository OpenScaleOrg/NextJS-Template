...
const HamburgerMenu = (
    ...
    return (
        <>
            ...
            <div className={`absolute flex md:hidden bg-slate-300 top-[60px] z-10 left-0 w-[260px] transition-transform duration-300 transform ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'} glass-container bg-opacity-50 backdrop-filter backdrop-blur-md rounded-r-lg shadow-lg h-[calc(100vh-60px)]`}>
                ...
                <div className="bg-main-background left-0 z-40 min-w-[260px] pb-14 max-w-[260px] h-full flex flex-col justify-between items-center gap-2">
                    <div className='w-full p-4 flex flex-col justify-start items-center'>
                        <Link href={'/studio'} type="button" className={`w-full h-[44px] md:p-4 p-4 md:gap-4 gap-4 text-[14px] rounded-[12px] flex items-center cursor-pointer ${index == 0 ? 'bg-gradient-to-r from-[#3B82F6] to-[#A855F7] text-main-background dark:text-main-foreground' : 'text-main-light dark:text-main-foreground'}`}>
                            {
                                index == 0 ? <Image src={'/icons/home.svg'} height={20} width={20} alt='home' /> : <Image src={'/icons/home.svg'} height={20} width={20} alt='home' />
                            }
                            <span className="font-medium">Home</span>
                        </Link>
                        <Link href={'/studio'} type="button" className={`w-full h-[44px] md:p-4 p-4 md:gap-4 gap-4 text-[14px] rounded-[12px] flex items-center cursor-pointer ${index == 1 ? 'bg-gradient-to-r from-[#3B82F6] to-[#A855F7] text-main-background dark:text-main-foreground' : 'text-main-light dark:text-main-foreground'}`}>
                            {
                        ...
                        <Link href={'/studio'} type="button" className={`w-full h-[44px] md:p-4 p-4 md:gap-4 gap-4 text-[14px] rounded-[12px] flex items-center cursor-pointer ${index == 3 ? 'bg-gradient-to-r from-[#3B82F6] to-[#A855F7] text-main-background dark:text-main-foreground' : 'text-main-light dark:text-main-foreground'}`}>
                            {
                    ...
                    <div className='w-full border-t-1 border-main-light/20 flex flex-col justify-start items-center'>
                        <div title="shirts" className={`w-full h-[60px] md:p-4 p-4 md:gap-4 gap-4 text-[14px] rounded-[12px] flex items-center cursor-pointer`}>
